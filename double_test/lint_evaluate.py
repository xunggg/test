...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path):
    ...
    adv_output_dir = os.path.join(lint_output_path, src_model_name, dataset_name, attack_method)
        ...
        for target_mname in victim_model_names:
            for ds_name, ds_path in victim_datasets:
                for attack_method in baseline_attack_methods.keys():
                    # if attack_method not in ['LINTDI-FGSM', 'LINTTI-FGSM', 'LINTDEM', 'LINTAdmix', 'LINTSIT']:
                    if attack_method not in ['LINTDEM-Conv', 'LINTAdmix-Conv', 'LINTSIT-Conv']:
                        continue
                    evaluate(lint_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path)
    print('===evaluate end===')
