...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path):
    mzoo = ModelZoo() 
    ...
    adv_output_dir = os.path.join(test_output_path, src_model_name, dataset_name, attack_method)
    ...
    prefix = '{},{},{},{:.4f},{:.4f},'.format(attack_method, src_model_name,
...
if __name__ == '__main__':
    ...
    # for src_mname in ['inception_v3']:
        ...
        # for target_mname in ['adv_inception_v3', 'ens_adv_inception_resnet_v2', 'inception_resnet_v2']:
            for ds_name, ds_path in victim_datasets:
                ...
                # for attack_method in ['DDVNI-FGSM']:
                    if attack_method in ['AA', 'TAA', 'BSI-FGSM']:
                        continue
                    # if attack_method not in ['TI-FGSM', 'Logit']:
                    # if attack_method not in ['TSIT']:
                    # if attack_method not in ['CFM']:
                    # if attack_method not in ['BAP']:
                    if attack_method not in ['LBAP']:
                    # if attack_method not in ['DDMI-FGSM']:
                    # if attack_method not in ['DDDI-FGSM']:
                    # if attack_method not in ['DDTI-FGSM']:
                    # if attack_method not in ['GTI-FGSM']:
                    # if attack_method not in ['SSA']:
                    # if attack_method not in ['Poincare']:
                    # if attack_method not in ['TBAP']:
                        continue
                    evaluate(test_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path)
    print('===evaluate end===')
