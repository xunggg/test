...
def evaluate(evaluation_file, holdout_model_name, ens_models, attakc_method, dataset_name, dataset_path):
    mzoo = ModelZoo() 
    ...
    adv_output_dir = os.path.join(ens_output_path, holdout_model_name, dataset_name, attack_method)
    ...
    prefix = '{},{},{},{:.4f},{:.4f},'.format(attack_method, str(ens_models), holdout_model_name
                                    ,am.error_rate(),
...
if __name__ == '__main__':
    ...
    # for holdout_mname in ['resnet50']:
    model_names = ['resnet50', 'vgg19', 'inception_v3', 'densenet121', 'wide_resnet50_2']
    # for holdout_mname in model_names: 
    # for holdout_mname in ['resnet50', 'vgg19', 'wide_resnet50_2']: 
    # for holdout_mname in ['densenet121', 'inception_v3']: 
    for holdout_mname in ['densenet121']: 
        ...
        for ds_name, ds_path in victim_datasets:
            ...
            # for attack_method in ['ENSVMI-FGSM']:
            # for attack_method in ['ENSBAP', 'ENSSIT']:
            # for attack_method in ['ENSDI-FGSM']:
            # for attack_method in ['DI-FGSM']:
            # for attack_method in ['ENSNI-FGSM']:
            # for attack_method in ['NI-FGSM', 'DDNI-FGSM']:
            # for attack_method in ['NI-FGSM', 'DINI-FGSM']:
            # for attack_method in ['SINI-FGSM', 'DDSINI-FGSM']:
            # for attack_method in ['SINI-FGSM', 'DISINI-FGSM']:
            # for attack_method in ['MI-FGSM', 'DDMI-FGSM']:
            # for attack_method in ['MI-FGSM', 'DIMI-FGSM']:
            for attack_method in ['ENSDI-FGSM', 'ENSMI-FGSM', 'ENSBAP', 'ENSTI-FGSM', 'ENSSIT', 'ENSPoincare']:
                evaluate(ens_evaluation_file, holdout_mname, ens_models, attack_method, ds_name, ds_path)
    print('===evaluate end===')
