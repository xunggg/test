...
import numpy as np
from torchdefenses.FS import defense
...
def evaluate(evaluation_file, holdout_model_name, ens_models, attakc_method, dataset_name, dataset_path, defense_name):
    ...
    # load train features
    clean_dname, clean_path = clean_libraries
    clean_ds = dzoo.load_dataset(clean_dname, clean_path)
    fs_defense = defense.Defense(model, clean_dataset=clean_ds)
    ...
    for fname in tqdm(adv_imgs):
        ...
        feature = load_one_img(fpath).unsqueeze(0)
        ...
        if fs_defense(feature):
            # adv
            found = found + 1
            white_confidences.append(F.one_hot(torch.LongTensor([src]), 1000))
            black_confidences.append(F.one_hot(torch.LongTensor([src]), 1000))
        else:
            adv_logits = model(feature.cuda())
...
if __name__ == '__main__':
    ...
    for holdout_mname in ['inception_v3']: 
        ...
        for ds_name, ds_path in victim_datasets:
            for defense_name in ['fs']:
                for attack_method in ['ENSDI-FGSM', 'ENSMI-FGSM', 'ENSBAP', 'ENSTI-FGSM', 'ENSSIT', 'ENSPoincare']:
                    evaluate(fs_evaluation_file, holdout_mname, ens_models, attack_method, ds_name, ds_path, defense_name)
    print('===evaluate end===')
