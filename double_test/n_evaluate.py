...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path, arg):
    ...
    adv_output_dir = os.path.join(n_output_path, str(arg), src_model_name, dataset_name, attack_method)
        ...
        for target_mname in victim_model_names:
            for ds_name, ds_path in victim_datasets:
                for attack_method in baseline_attack_methods.keys():
                    for arg in [20, 30, 40, 50, 60, 70, 80]:
                    # for arg in [10,]: 
                        # if attack_method not in ['SIT', 'LBAP', 'LBAP-Conv']:
                        if attack_method not in ['LBAP-Conv']:
                            continue
                        evaluate(n_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path, arg)
    print('===evaluate end===')
