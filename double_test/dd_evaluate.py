...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path):
    ...
    adv_output_dir = os.path.join(dd_output_path, src_model_name, dataset_name, attack_method)
        ...
        for target_mname in victim_model_names:
            for ds_name, ds_path in victim_datasets:
                ...
                # for attack_method in ['DDVNI-FGSM']:
                    ...
                    # if attack_method not in ['SSA', 'SIT', 'BAP']:
                    if attack_method not in ['TI-FGSM', 'DI-FGSM', 'SIT', 'DDTI-FGSM', 'DDDI-FGSM', 'DDSIT']:
                        continue
                    evaluate(dd_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path)
    print('===evaluate end===')
