...
def main():
    ...
    # for mixup in [0.0, 0.05, 0.1, 0.15, 0.20, 0.25]:
    for mixup in [0.15, 0.20, 0.25]:
    # for mixup in [0.0, 0.05, 0.1]:
        for mname in model_names:
            print('model {} generates adversarial examples...'.format(mname))
    
            adv_output_dir = os.path.join(gamma_output_path, str(mixup), mname)
            ...
            for (dname, dpath), (fbname, fbpath) in zip(victim_datasets, feature_libraries):
                ...
                adv_output_dir = os.path.join(gamma_output_path, str(mixup), mname, dname)
                if not os.path.exists(adv_output_dir):
                    os.makedirs(adv_output_dir)
                ...
                for i, (attack_name, attack_args) in enumerate(baseline_attack_methods.items()):
                    ...
                    adv_output_dir = os.path.join(gamma_output_path, str(mixup), mname, dname, attack_name)
                    if not os.path.exists(adv_output_dir):
                        os.makedirs(adv_output_dir)
                    ...
                    if attack_name == 'DI-FGSM':
                        ...
                        attack = attacks.bap.BAP(model,
                                ...
                                alpha=alpha,
                                decay=attack_args['decay_factor'],
                                n=10, mixup_num=3, mixup_weight=0.4, gamma=mixup, mixup_ratio=0.7)
                        #        n=attack_args['n'])
                        # targeted
                        attack.set_mode_targeted_by_label()
                    else:
