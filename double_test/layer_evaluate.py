...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path, arg):
    ...
    adv_output_dir = os.path.join(layer_output_path, str(arg), src_model_name, dataset_name, attack_method)
...
if __name__ == '__main__':
    ...
    # for src_mname in ['vgg19']:
        mzoo = ModelZoo()
        if src_mname == 'resnet50':
            layers = mzoo.res50_feature_layers
        elif src_mname == 'densenet121':
            layers = mzoo.dense121_feature_layers
        elif src_mname == 'vgg19':
            ...
            for ds_name, ds_path in victim_datasets:
                for attack_method in baseline_attack_methods.keys():
                    if attack_method not in ['LBAP-Conv']:
                        continue
                    for index, arg in enumerate(layers): 
                        evaluate(layer_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path, arg)
    print('===evaluate end===')
