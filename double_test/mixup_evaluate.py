...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path, arg):
    ...
    adv_output_dir = os.path.join(mixup_output_path, str(arg), src_model_name, dataset_name, attack_method)
...
if __name__ == '__main__':
    # for src_mname in ['resnet50', 'vgg19', 'wide_resnet50_2']:
    # for src_mname in ['vgg19', 'wide_resnet50_2']:
    for src_mname in ['densenet121']:
        for target_mname in victim_model_names:
            for ds_name, ds_path in victim_datasets:
                for attack_method in baseline_attack_methods.keys():
                    for arg in range(0, 11):
                        if attack_method not in ['LBAP']:
                            continue
                        evaluate(mixup_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path, arg)
    print('===evaluate end===')
