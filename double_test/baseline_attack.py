...
def main():
    ...
    # for mname in ['densenet121', 'wide_resnet50_2', 'vgg19']
    for mname in model_names: 
        print('model {} generates adversarial examples...'.format(mname))

        adv_output_dir = os.path.join(adv_output_path, mname)
        ...
        for (dname, dpath), (fbname, fbpath) in zip(victim_datasets, feature_libraries):
            ...
            adv_output_dir = os.path.join(adv_output_path, mname, dname)
            if not os.path.exists(adv_output_dir):
                os.makedirs(adv_output_dir)
            ...
            for i, (attack_name, attack_args) in enumerate(baseline_attack_methods.items()):
                ...
                if attack_name not in ['MI-FGSM', 'VMI-FGSM', 'VNI-FGSM', 'SSA', 'SIT', 'DI-FGSM', 'TI-FGSM', 'PBAP']:
                # if attack_name not in ['SIT', 'BAP']:
                    continue

                if mname == 'densenet121' and attack_name not in ['SSA', 'TBAP', 'SIT']:
                    continue
                ...
                adv_output_dir = os.path.join(adv_output_path, mname, dname, attack_name)
                if not os.path.exists(adv_output_dir):
                    os.makedirs(adv_output_dir)
                ...
                if attack_name == 'DI-FGSM':
                    ...
                    attack = attacks.bap.BAP(model,
                            ...
                            n=30, mixup_num=3, mixup_weight=0.4, mixup_ratio=0.7, beta1=0.0, beta2=0.1)
                    #        n=attack_args['n'])
                    # targeted
                    attack.set_mode_targeted_by_label()
