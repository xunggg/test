...
from model_nrp import ModelNRP
...
def evaluate(evaluation_file, holdout_model_name, ens_models, attakc_method, dataset_name, dataset_path, pure_name, dynamic=False):
    pure_zoo = ModelNRP(dynamic=dynamic)
    mzoo = ModelZoo() 

    pmodel = pure_zoo.pick_model(pure_name)
    ...
    for holdout_mname in ['inception_v3']: 
        ...
        for ds_name, ds_path in victim_datasets:
            ...
            for attack_method in ['ENSDI-FGSM', 'ENSMI-FGSM', 'ENSBAP', 'ENSTI-FGSM', 'ENSSIT', 'ENSPoincare']:
                evaluate(nrp_evaluation_file, holdout_mname, ens_models, attack_method, ds_name, ds_path, 'nrp')
    print('===evaluate end===')
