...
def main():
    ...
    for mixup in range(1, 30, 2): 
        ...
        for mname in ['resnet50', 'wide_resnet50_2', 'vgg19']:
            ...
            adv_output_dir = os.path.join(kern_output_path, str(mixup), mname)
            ...
            for (dname, dpath), (fbname, fbpath) in zip(victim_datasets, feature_libraries):
                ...
                adv_output_dir = os.path.join(kern_output_path, str(mixup), mname, dname)
                if not os.path.exists(adv_output_dir):
                    os.makedirs(adv_output_dir)
                ...
                for i, (attack_name, attack_args) in enumerate(baseline_attack_methods.items()):
                    ...
                    adv_output_dir = os.path.join(kern_output_path, str(mixup), mname, dname, attack_name)
                    if not os.path.exists(adv_output_dir):
                        os.makedirs(adv_output_dir)
                    ...
                    if attack_name == 'DI-FGSM':
                        ...
                        attack = attacks.llbap.LLBAP(model,
                            ...
                            alpha=alpha,
                            decay=attack_args['decay_factor'],
                            n=10, emb_aug_type='conv', kern_size=mixup)
                        # targeted
                        attack.set_mode_targeted_by_label()
                    else:
