...
def main():
    ...
    for mname in model_names:
        ...
        adv_output_dir = os.path.join(smo_output_path, mname)
        ...
        for (dname, dpath), (fbname, fbpath) in zip(victim_datasets, feature_libraries):
            ...
            adv_output_dir = os.path.join(smo_output_path, mname, dname)
            if not os.path.exists(adv_output_dir):
                os.mkdir(adv_output_dir)
            ...
            for i, (attack_name, attack_args) in enumerate(baseline_attack_methods.items()):
                ...
                # if attack_name not in ['MBAP', 'BAP']:
                if attack_name not in ['MBAP']:
                    continue
                ...
                adv_output_dir = os.path.join(smo_output_path, mname, dname, attack_name)
                if not os.path.exists(adv_output_dir):
                    os.mkdir(adv_output_dir)
                ...
                if attack_name == 'DI-FGSM':
                    ...
                    # targeted
                    attack.set_mode_targeted_by_label()
                elif attack_name == 'MBAP':
                    # alpha = 1.0 
                    alpha = attack_args['eps'] / attack_args['max_iter']
                    attack = attacks.mbap.MBAP(model,
                            eps=attack_args['eps'],
                            steps=attack_args['max_iter'],
                            alpha=alpha,
                            decay=attack_args['decay_factor'],
