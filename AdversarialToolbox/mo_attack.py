...
def main():
    ...
    for mname in model_names:
        ...
        adv_output_dir = os.path.join(mo_output_path, mname)
        ...
        for (dname, dpath), (fbname, fbpath) in zip(victim_datasets, feature_libraries):
            ...
            adv_output_dir = os.path.join(mo_output_path, mname, dname)
            if not os.path.exists(adv_output_dir):
                os.mkdir(adv_output_dir)
            ...
            for i, (attack_name, attack_args) in enumerate(baseline_attack_methods.items()):
                ...
                if attack_name not in ['TBAP', 'BAP']:
                    continue
                ...
                adv_output_dir = os.path.join(mo_output_path, mname, dname, attack_name)
                if not os.path.exists(adv_output_dir):
                    os.mkdir(adv_output_dir)
