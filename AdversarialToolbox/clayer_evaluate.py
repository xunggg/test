...
def evaluate(evaluation_file, src_model_name, target_model_name, attakc_method, dataset_name, dataset_path):
    ...
    adv_output_dir = os.path.join(mlayer_output_path, src_model_name, dataset_name, attack_method)
...
if __name__ == '__main__':
    # for src_mname in ['resnet50', 'densenet121', 'vgg19']:
    for src_mname in ['wide_resnet50_2']:
        for target_mname in victim_model_names:
            for ds_name, ds_path in victim_datasets:
                for attack_method in baseline_attack_methods.keys():
                    if attack_method not in ['LBAP-parallel', 'LBAP-mixconv', 'LBAP-convmix', 'LBAP-convconv', 
                        'LBAP-mixmix', 'LBAP-concatenate']:
                        continue
                    evaluate(mlayer_evaluation_file, src_mname, target_mname, attack_method, ds_name, ds_path)
    print('===evaluate end===')
