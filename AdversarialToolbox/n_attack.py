...
def main():
    ...
    # for mname in ['resnet50', 'densenet121', 'wide_resnet50_2']:
        for n in [1, 20, 30, 40, 50, 60, 70, 80, 90]:
            print('model {} with {} generates adversarial examples...'.format(mname, n))
    
            adv_output_dir = os.path.join(n_output_path, str(n), mname)
            ...
            for (dname, dpath), (fbname, fbpath) in zip(victim_datasets, feature_libraries):
                ...
                adv_output_dir = os.path.join(n_output_path, str(n), mname, dname)
                if not os.path.exists(adv_output_dir):
                    os.makedirs(adv_output_dir)
                ...
                for i, (attack_name, attack_args) in enumerate(baseline_attack_methods.items()):
                    ...
                    # if attack_name not in ['LBAP', 'SIT']:
                    ...
                    adv_output_dir = os.path.join(n_output_path, str(n), mname, dname, attack_name)
                    if not os.path.exists(adv_output_dir):
                        os.makedirs(adv_output_dir)
                    ...
                    if attack_name == 'DI-FGSM':
                        ...
                        attack = attacks.bap.BAP(model,
                                ...
                                alpha=alpha,
                                decay=attack_args['decay_factor'],
                                n=n, mixup_num=4, mixup_weight=0.4)
                        #        n=attack_args['n'])
                        # targeted
                        attack.set_mode_targeted_by_label()
